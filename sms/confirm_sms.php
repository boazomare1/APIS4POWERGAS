    ...
    try {
        ...
        if($stmt->rowCount() > 0){
            ...
            if ($signature_data === false) {
                ...
                return array("success" => "5", "message" => "Invalid signature data");
            }
            ...
            if($write_result === false){
                ...
                return array("success" => "3", "message" => "Failed to save signature image. Path: $path, Error: $error_msg");
            }
            ...
            if($cheque_image != null && !empty($cheque_image)){
                ...
                if ($cheque_data === false) {
                    ...
                    return array("success" => "6", "message" => "Invalid cheque image data", "image_url"=>$final_path);
                } else {
                    ...
                    if($cheque_write === false){
                        ...
                        return array("success" => "7", "message" => "Failed to save cheque image", "image_url"=>$final_path);
                    } else {
                        return array("success" => "1", "message" => "Verification code successful", "image_url"=>$final_path, "cheque_image"=>$final_path2);
                    }
                }
            } else {
                return array("success" => "1", "message" => "Verification code successful", "image_url"=>$final_path, "cheque_image"=>null);
            }

        }else{
            return array("success" => "0", "message" => "Verification not successfull please try again");
        }

    } catch (Exception $e) {
        print_r($e);
        http_response_code(404);
        return array("success" => "2", "message" => "error occured");
    }
}
